...
func (c Card) CreatePaymentIntent(currency string, amount int) (*stripe.PaymentIntent, string, error) {
	...
	// in case you want to add metadata
...
// RetrievePaymentIntend gets an existing payment intent by id
func (c *Card) RetrievePaymentIntend(id string) (*stripe.PaymentIntent, error) {
	stripe.Key = c.Secret
...
func (c *Card) SubscribeToPlan(cust *stripe.Customer, plan, email, last4, cardType string) (string, error) {
	...
	subsciptions, err := sub.New(params)
	if err != nil {
		return "", err
	}
	return subsciptions.ID, nil
}
...
func cardErrorMessage(code stripe.ErrorCode) string {
	...
	case stripe.ErrorCodeCardDeclined:
		msg = "Your ward was declined"
	case stripe.ErrorCodeExpiredCard:
		msg = "Your ward is expired"
